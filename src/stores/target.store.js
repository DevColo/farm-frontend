...
export const useTargetStore = defineStore('target', {
  ...
  actions: {
    ...
    async fetchPastureById(id) {
      this.loading = true
      
      try {
        const token = localStorage.getItem('user_token')
        const response = await axios.get(`/api/get-pasture/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
        ...
        if (response.data.success) {
          this.pasture = response.data.data
        } else {
          this.pasture = null
        }
      } catch (error) {
        this.handleError(error, 'Failed to fetch pasture details')
        this.pasture = null
      } finally {
        this.loading = false
      }
