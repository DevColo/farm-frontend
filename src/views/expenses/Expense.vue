...
const itemTypes = [
  ...
  { value: 'Veterinary Supply', label: 'Veterinary Supply'},
...
const filteredExpenses = computed(() => {
  ...
  filtered.sort((a, b) => {
    ...
    } else if (sortField.value === 'total_cost') {
      aValue = calculateTotalCost(a.quantity, a.unit_price)
      bValue = calculateTotalCost(b.quantity, b.unit_price)
...
async function confirmDelete(id, name) {
  if (confirm(`Are you sure you want to delete expense record for ${name}? This action cannot be undone.`)) {
    loading.value = true
    try {
...
async function handleSubmit(e) {
  ...
  try {
    const payload = {
      item: currentExpense.value.item?.value || currentExpense.value.item,
      name: currentExpense.value.name,
      quantity: currentExpense.value.quantity,
      unit_price: currentExpense.value.unit_price,
      farm_id: currentExpense.value.farm_id?.value || currentExpense.value.farm_id,
      purchased_by: currentExpense.value.purchased_by,
      purchase_date: currentExpense.value.purchase_date,
    }
...
const expenseStats = computed(() => {
  ...
  const totalAmount = expenseStore.expenses.reduce((sum, expense) => 
    sum + calculateTotalCost(expense.quantity, expense.unit_price), 0
  )
  ...
  return {
    ...
    lastPurchaseDate: sorted[0]?.purchase_date,
  }
})
</script>
...
<template>
  <div class="position-relative">
    ...
    <CBreadcrumb class="mb-4">
      ...
      <CBreadcrumbItem active>Expenses</CBreadcrumbItem>
    </CBreadcrumb>
      ...
      <CCard class="shadow-sm border-0">
        ...
        <CCardHeader class="bg-white border-bottom">
          <div class="d-flex justify-content-between align-items-center">
            <div>
              <h5 class="mb-1">
                <i class="fas fa-dollar-sign me-2 text-danger"></i>Expenses
              </h5>
            ...
            <CTable hover class="mb-0 modern-table">
              <CTableHead class="table-light">
                <CTableRow>
                  ...
                  <CTableHeaderCell class="sortable" @click="sortBy('unit_price')">
                    Unit Price (RWF) {{ getSortIcon('unit_price') }}
                  </CTableHeaderCell>
                  <CTableHeaderCell class="sortable" @click="sortBy('total_cost')">
                    Total Cost (RWF){{ getSortIcon('total_cost') }}
                  </CTableHeaderCell>
                  <CTableHeaderCell class="sortable" @click="sortBy('farm')">
                    Farm {{ getSortIcon('farm') }}
                  </CTableHeaderCell>
                  <CTableHeaderCell class="sortable" @click="sortBy('purchased_by')">
                    Purchased By {{ getSortIcon('purchased_by') }}
                  </CTableHeaderCell>
                  <CTableHeaderCell class="sortable" @click="sortBy('purchase_date')">
              ...
              <CTableBody>
                <CTableRow v-for="expense in paginatedExpenses" :key="expense.id" class="table-row">
                  ...
                  <CTableDataCell>
                    {{ parseFloat(expense.unit_price || 0).toFixed(2) }}
                  </CTableDataCell>
                  <CTableDataCell>
                    <strong>{{ calculateTotalCost(expense.quantity, expense.unit_price).toFixed(2) }}</strong>
                  </CTableDataCell>
                  <CTableDataCell>
                    <router-link
                  ...
                  <CTableDataCell>
                    <div class="d-flex gap-1">
                      ...
                      <CButton
                        ...
                        @click="confirmDelete(expense.id, expense.name)"
                      >
    ...
    <CModalBody class="p-0">
      ...
      >
        <CRow class="g-3">
          ...
          <CCol md="6">
            <CFormLabel for="unit_price" class="fw-semibold">Unit Price <span style="color: red;">*</span></CFormLabel>
            <CInputGroup>
              ...
              <CFormInput 
                ...
                type="number" 
                step="0.01"
                v-model="currentExpense.unit_price" 
                placeholder="0.00"
                required 
              />
            </CInputGroup>
            <CFormFeedback invalid>Unit price is required.</CFormFeedback>
          </CCol>

          <CCol md="12" v-if="currentExpense.quantity && currentExpense.unit_price">
            <div class="alert alert-info">
              <strong>Total Cost:</strong> ${{ calculateTotalCost(currentExpense.quantity, currentExpense.unit_price).toFixed(2) }}
            </div>
          </CCol>
          ...
          <CCol md="12">
            <CFormLabel for="purchased_by" class="fw-semibold">Purchased By <span style="color: red;">*</span></CFormLabel>
            <CFormInput 
              id="purchased_by" 
              type="text" 
              v-model="currentExpense.purchased_by" 
              placeholder="Enter purchaser name"
              required 
            />
            <CFormFeedback invalid>Purchased by is required.</CFormFeedback>
          </CCol>

          <CCol md="12">
            <CFormLabel for="purchase_date" class="fw-semibold">Purchase Date <span style="color: red;">*</span></CFormLabel>
            <CFormInput 
              id="purchase_date" 
              type="date" 
              v-model="currentExpense.purchase_date" 
              required 
            />
            <CFormFeedback invalid>Purchase date is required.</CFormFeedback>
          </CCol>
        </CRow>
      </CForm>
    </CModalBody>
